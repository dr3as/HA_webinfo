...
} elseif (time() - $_SESSION['last_activity'] > ($SESSION_TIMEOUT ?? 3600)) {
    session_unset();
    session_destroy();
}
$_SESSION['last_activity'] = time();
...
function ha_get($sensor_id){
    ...
    $url = rtrim($HA_URL, '/') . '/' . ltrim($sensor_id, '/');
    $opts = ['http'=>[
        'method'=>"GET",
...
<div class="category">
    ...
    <div class="item-container">
        <div class="item">
            ...
            <span>Kontor: <?= htmlspecialchars($data['kontor_temp']) ?> °C</span>
        </div>
        <div class="item">
            <span class="icon">🌡️</span>
            <span>Ute: <?= htmlspecialchars($data['ute_temp']) ?> °C</span>
        </div>
    </div>
</div>
...
<div class="category">
    ...
    <div class="item-container">
        <div class="item">
            <span class="icon">☁️</span>
            <span>Forhold: <?= htmlspecialchars($data['condition_today']) ?></span>
        </div>
        <div class="item">
            <span class="icon">💧</span>
            <span>Nedbør: <?= htmlspecialchars($data['precipitation_today']) ?> mm (<?= htmlspecialchars($data['precipitation_probability_today']) ?>%)</span>
        </div>
        <div class="item">
            <span class="icon" style="color:<?= $data['temperature_high_color'] ?>;">&#9650;</span>
            <span style="color:<?= $data['temperature_high_color'] ?>;"> Høy temp: <?= htmlspecialchars($data['temperature_high_today']) ?> °C</span>
        </div>
        <div class="item">
            <span class="icon" style="color:<?= $data['temperature_low_color'] ?>;">&#9660;</span>
            <span style="color:<?= $data['temperature_low_color'] ?>;"> Lav temp: <?= htmlspecialchars($data['temperature_low_today']) ?> °C</span>
        </div>
    </div>
</div>
...
<div class="category">
    ...
    <?php if (count($calendar_events)):
        foreach ($calendar_events as $event): ?>
            <div class="item">
                <span class="icon">📅</span>
                <span><strong><?= htmlspecialchars($event['title']) ?></strong></span>
                <?php if ($event['start'] || $event['end']): ?>
                    <p class="subinfo"><?= htmlspecialchars($event['start']) ?> - <?= htmlspecialchars($event['end']) ?></p>
                <?php endif; ?>
            </div>
        <?php endforeach;
    else: ?>
...
<div class="category">
    ...
    <div class="item-container">
        <div class="item">
            <span class="icon">🚆</span>
            <div>
                <p><strong>Avvik:</strong> <?= htmlspecialchars($avvik_status) ?></p>
                <p><strong>Forsinkelser:</strong> <?= htmlspecialchars($forsinkelse_status) ?></p>
            </div>
        </div>
    </div>
</div>
